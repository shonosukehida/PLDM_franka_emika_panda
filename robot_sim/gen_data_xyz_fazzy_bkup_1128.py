...
class FrankaDatasetGenerator:
    def __init__(self, config):
        ...
        print("sample_method:", self.SAMPLE_METHOD)
        self.specify_init_position = config['specify_init_position']
        
        self.eval_only = self.config['eval_only']

        self.SAVE_PATH = (
            f"pldm_envs/franka/presaved_datasets/val_pairs_{self.PAIRS}_ep_{self.EPISODES_PER_PAIR}_timestep_{self.STEPS_PER_EPISODE}_sample_{self.SAMPLE_METHOD}"
            if self.IS_VAL else
            f"pldm_envs/franka/presaved_datasets/pairs_{self.PAIRS}_ep_{self.EPISODES_PER_PAIR}_timestep_{self.STEPS_PER_EPISODE}_sample_{self.SAMPLE_METHOD}"
        )
                ...
                for idx in range(self.STEPS_PER_EPISODE):
                    if idx % self.target_sampling_step == 0:
                        ...
                        if idx == 0: #初期IKの
                            ...
                            if self.SAMPLE_METHOD == 'uniform':
                            ...
                            elif self.SAMPLE_METHOD == 'towards_bluebox':
                                current_pos = self.env.get_ee_position()
                                ...
                                near_threshold = tb_cfg.get('near_threshold', 5.03)
